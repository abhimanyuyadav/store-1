...
export async function POST(request: Request) {
  try {
    ...
    const result = await getConvexServerClient().action((api as any).auth.setupFirstAdmin, body);
    ...
    return NextResponse.json({ error: error instanceof Error ? error.message : "Could not set up administrator." }, { status: 400 });
  }
}
